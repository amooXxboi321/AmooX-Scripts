...
use smithay::{
    ...
    input::{
        keyboard::LedState,
        pointer::{CursorImageAttributes, CursorImageStatus, MotionEvent},
    },
    ...
    utils::{Logical, Monotonic, Point, Scale, Time, Transform, SERIAL_COUNTER as SCOUNTER},
    wayland::{
        compositor,
...
pub fn run_udev() {
    ...
    // Ensure a visible cursor at startup even if pointer devices were already present.
    state.cursor_status = CursorImageStatus::default_named();
    spawn_budgiex_shell(&state);
    ...
    event_loop
        ...
        .insert_source(libinput_backend, move |mut event, _, data| {
            ...
            if let InputEvent::DeviceAdded { device } = &mut event {
                ...
                let has_pointer = device.has_capability(DeviceCapability::Pointer);
                ...
                if has_pointer {
                    // Ensure we have a visible cursor when a pointer device is present.
...
fn spawn_budgiex_shell<BackendData: Backend + 'static>(state: &AnvilState<BackendData>) {
    let Some(socket_name) = state.socket_name.as_ref() else {
        warn!("No wayland socket name available; skipping BudgieX shell spawn");
        return;
    };
    let runtime_dir = std::env::var("XDG_RUNTIME_DIR").unwrap_or_else(|_| "/run/user/1000".into());
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/amooxxboi".into());
    let launcher_path = format!("{}/.local/bin/budgiex-launcher", home);

    let mut cmd = Command::new(&launcher_path);
    cmd.env("WAYLAND_DISPLAY", socket_name)
        .env("XDG_RUNTIME_DIR", runtime_dir)
        .env("XDG_SESSION_TYPE", "wayland")
        .env("XDG_CURRENT_DESKTOP", "BudgieX")
        .env("XDG_SESSION_DESKTOP", "BudgieX");

    match cmd.spawn() {
        Ok(_) => info!(wayland_socket = socket_name, path = %launcher_path, "Spawned BudgieX shell"),
        Err(err) => error!(%err, path = %launcher_path, "Failed to spawn BudgieX shell"),
    }
}
...
impl AnvilState<UdevData> {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            info!(?wl_mode, ?position, "Output mode selected");
            info!(
                geometry = ?self.space.output_geometry(&output),
                "Output geometry after map"
            );
            if let Some(geo) = self.space.output_geometry(&output) {
                if geo.size.w == 0 || geo.size.h == 0 {
                    warn!(?geo, "Output geometry has zero size");
                }
    ...
    fn frame_finish(&mut self, dev_id: DrmNode, crtc: crtc::Handle, metadata: &mut Option<DrmEventMetadata>) {
        ...
        let Some(frame_duration) = output.current_mode().map(|mode| {
            let refresh_millihz = mode.refresh.max(1) as f64;
            let refresh_hz = refresh_millihz / 1000.0;
            Duration::from_secs_f64(1.0 / refresh_hz)
        })
        else {
            return;
        };
    ...
    fn render_surface(&mut self, node: DrmNode, crtc: crtc::Handle, frame_target: Time<Monotonic>) {
        ...
        if reschedule {
            let output_refresh_millihz = match output.current_mode() {
            ...
            let refresh = output_refresh_millihz.max(1) as u64;
            let frame_nanos = 1_000_000_000_000u64 / refresh;
            let next_frame_target = frame_target + Duration::from_nanos(frame_nanos);
...
fn render_surface<'a>(
    ...
    let output_geometry = match space.output_geometry(output) {
        Some(geo) if geo.size.w > 0 && geo.size.h > 0 => Some(geo),
        _ => {
            static WARN_ONCE: Once = Once::new();
            WARN_ONCE.call_once(|| {
                warn!("Output geometry unavailable or invalid during render_surface");
            });
            None
        }
    };
    ...
    if !pointer_location.x.is_finite() || !pointer_location.y.is_finite() {
        warn!(?pointer_location, "Pointer location is not finite");
    } else if let Some(output_geometry) = output_geometry {
